...
func (h *handlers) GetRegisteredCourses(c echo.Context) error {
	...
	if err := tx.SelectContext(c.Request().Context(), &courses, query, StatusClosed, userID); err != nil {
		c.Logger().Error(err)
		return c.NoContent(http.StatusInternalServerError)
	}
	...
	for _, course := range courses {
		var teacher User
		if err := tx.GetContext(c.Request().Context(), &teacher, "SELECT * FROM `users` WHERE `id` = ?", course.TeacherID); err != nil {
			c.Logger().Error(err)
			return c.NoContent(http.StatusInternalServerError)
		}
...
func (h *handlers) RegisterCourses(c echo.Context) error {
	...
	for _, course := range newlyAdded {
		_, err = tx.ExecContext(c.Request().Context(), "INSERT INTO `registrations` (`course_id`, `user_id`) VALUES (?, ?) ON CONFLICT DO UPDATE SET `course_id` = EXCLUDED.course_id, `user_id` = EXCLUDED.user_id", course.ID, userID)
		if err != nil {
			c.Logger().Error(err)
			return c.NoContent(http.StatusInternalServerError)
		}
...
func (h *handlers) SubmitAssignment(c echo.Context) error {
	...
	if _, err := tx.ExecContext(c.Request().Context(), "INSERT INTO `submissions` (`user_id`, `class_id`, `file_name`) VALUES (?, ?, ?) ON CONFLICT DO UPDATE SET `file_name` = EXCLUDED.file_name", userID, classID, header.Filename); err != nil {
		c.Logger().Error(err)
		return c.NoContent(http.StatusInternalServerError)
	}
...
func (h *handlers) GetAnnouncementList(c echo.Context) error {
	...
	if err := tx.SelectContext(c.Request().Context(), &announcements, query, args...); err != nil {
		c.Logger().Error(err)
		return c.NoContent(http.StatusInternalServerError)
	}
